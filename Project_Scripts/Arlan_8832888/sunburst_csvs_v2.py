#this script takes a level 6 taxonomy tsv file output by Qiime2/Biom as input
...
parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter,
                                 usage = "sunburst_csvs.py directory file",
                                 description = textwrap.dedent('''\
                                    Example:
                                    python sunburst_csvs.py /data/gencore/qiime-taxonomy/ level6.tsv

                                    the directory is the folder containing the input file; output will be written here also
                                    the file is the filename of the level 6 taxonomy, assumed to be located in the directory
                                    '''))
...
for s in range(1,len(sArray[1])):
    ...
    for x in range(3, len(sArray)):
        if sArray[x][s] != "0.0":
            ...
            prev=""
            spcPresent=False
            if (taxon[5].removeprefix(prefix_dict[5][0]) != "__") and (taxon[5].removeprefix(prefix_dict[5][0]) != ""):
                spcPresent=True
            for t in range(0,6):
                entry = taxon[t]
                ...
                if (entry == "") or (entry == "__"):
                    if spcPresent:
                        count_data[place].append(prefix_dict[t][0]+"unspecified")
                    elif (prev == 'other') or (prev == None):
                        count_data[place].append(None)
                        prev=None
                    elif (t == 5):
                        count_data[place].append("unspecified")
                    else:
    ...
    df = pd.DataFrame(data=count_data, index=None, columns=["kingdom", "phylum", "class", "order", "family", "genus", "abundance"])
    df["abundance"] = pd.to_numeric(df["abundance"])
    df = (df.groupby(["kingdom", "phylum", "class", "order", "family", "genus"], sort=False, as_index=False, dropna=False)
            .agg({'abundance':'sum'})
