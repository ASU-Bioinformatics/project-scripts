...
#SBATCH -t 0-16:00                         # estimated time needed (dada2 can take a while)
#SBATCH --mem=128G
...
while [ : ]; do
  case "$1" in
    ...
    -m | --metadata)
        ...
        ;;
    -c | --categoricals)
        echo "The categorical metadata columns are '$2'"
        categoricals=$2
        shift 2
        ;;
    -n | --numericals)
        echo "The numerical metadata columns are '$2'"
        numericals=$2
        shift 2
        ;;
    -s | --strandedness)
...
if [ "$help" == "TRUE" ]; then
  ...
  options:
    ...
    [ -s  |   --strandedness  |   the strandedness of the sequencing. Allowable options are p, s, and ps (paired, single, and paired load with single DADA2) ]
