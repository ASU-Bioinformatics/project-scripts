...
params='INFO/DP<10 || QD<30 || SOR>3'
bcftools filter -e "$params" -O v \
                -o "$outDir"/"$name".genotyped.filtered.gatk.g.vcf \
                "$outDir"/"$name".genotyped.gatk.g.vcf

# these second two filters aren't doing anything but the first one cut out about 1/2 the snps
params='FMT/DP<3 | FMT/GQ<20'
bcftools filter -S . -e $params -O v \
                -o "$outDir"/"$name".genotyped.filtered2.gatk.g.vcf \
                "$outDir"/"$name".genotyped.filtered.gatk.g.vcf

params='F_MISSING > 0.2 || MAF <= 0.02'
bcftools filter -e  $params -O v \
                -o "$outDir"/"$name".genotyped.filtered2.gatk.g.vcf \
                "$outDir"/"$name".genotyped.filtered3.gatk.g.vcf
