...
metadata <- read.delim("/Volumes/Gencore/analysis_projects/6209010_Keaton_Meta/Publication-Preparation/metadata.txt", sep="\t")
metadata <- metadata[order(metadata$SampleID),]
#metadata$PlantType <- as.character(metadata$PlantType)

setwd("/Volumes/Gencore/sftp/t_sandrin/6209010_Metaomics/4.humann-assembly-free-analysis/1.metaphlan-classifications/3.clustered-heatmaps/species-level")
...
#### PLOT PRINCIPAL COORDINATE COLORED BY METADATA ####
sc <- viridis::scale_color_viridis("viridis", discrete = FALSE)
setwd("/Volumes/Gencore/analysis_projects/6209010_Keaton_Meta/Publication-Preparation")
...
simpleRDA <- rda(bug_mat ~ Sex + Race + DiseaseGroup, data=metadata)
plot(simpleRDA, scaling = 2)
