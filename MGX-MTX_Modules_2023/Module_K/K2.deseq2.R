...
option_list <- list(
  make_option(c("-d", "--directory"), type="character",
              default="/Volumes/Gencore/analysis_projects/6209010_Keaton_Meta/RNA-Metagenomics/human-removed/differential-expression",
...
# standardize names (gcnts names are taken from file names and contain extraneous information; comparison names are shorter IDs typically)
...
sapply(names(resSig),
       function (x) {
         ...
         write.table(data, file=paste0(x,".deg.all.deseq.txt"), quote=FALSE, sep="\t", row.names = TRUE, col.names = NA)
       })
