...
option_list <- list(
  make_option(c("-d", "--directory"), type="character",
              default="/data/gencore/analysis_projects/6078853_Otak_RNA/differential-expression",
              help="path to count matrix and template information"),
  make_option(c("-g", "--genes"), type="character",
              default="/data/gencore/analysis_projects/6078853_Otak_RNA/coassembly-contig-annotations/htseqcounts/transcript.count.matrix.tsv",
              help="count matrix file name"),
  make_option(c("-c", "--comparisons"), type="character",
              default="/data/gencore/analysis_projects/6078853_Otak_RNA/comparisons.csv",
...
top.qlflist <- sapply(names(qfitlist),
                      function (x) {
                        topTags(qlflist[[x]], n = gene.constant, p.value = 0.05)},
                      simplify = FALSE,
                      USE.NAMES = TRUE)
