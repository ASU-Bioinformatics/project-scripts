...
for condition in conditions:
    for comp in compSplit:
        print(comp)
        comp = comp.rstrip()
        edgeRF = open(comp + ".all.edgeR.txt")
        ...
        noiseqF = open(comp + ".deg.all.genes.noiseq.txt")
        ...
        if (noiseqList != ['\n']):
            for line in noiseqList:
                ...
                if abs(float(nsplit[nLogField])) >= log2fc :
                    ...
                    continue
        # sometimes there are NA fields in this file that are annoying to deal with
        # they don't have a p-value to work with, because it couldn't be calculated
        # so I used grep -v 'NA' to get rid of the lines with NA content and use that file
        deseq2F = open(comp + ".deg.allGenes.noNA.deseq.txt")
        ...
        with open(("merged_deg.allGenes." + comp + "." + condition + "." + str(log2fc) + ".stats.csv"), 'w', newline='') as file:
